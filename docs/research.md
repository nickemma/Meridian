...
The repository now includes `cmd/meridian-load`, which writes an operation-level JSONL history and a percentile summary, `cmd/meridian-check`, `bench/workloads/mixed.json`, and `scripts/netem.sh`. The checker exhaustively searches legal linearizations for a bounded, single-register strong history; it is useful for fault smoke runs but is not a Porcupine-scale checker. The current driver is closed-loop and supports only simple get/put mixes. It is useful for development smoke runs, but it is not the pre-registered open-loop, YCSB-compatible harness and cannot answer Q1–Q3 alone. No external baseline, fault timeline, independent large-history checker, durable gossip outbox, or retained result exists.
...
Causal reads and writes now wait only until their gRPC or request deadline for an unavailable vector or Raft-index dependency. A timeout is returned as `DEADLINE_EXCEEDED`; the service does not expose or create a value ahead of that dependency. The integration test supplies a committed strong write's Raft index to a causal read of the same key, and a causal unit test exercises both deadline expiry and release after the local watermark advances. The eventual replica test now models a partition, duplicate/reordered healing messages, and preservation of concurrent siblings. These are semantic tests, not partition availability measurements.

The same integration test also verifies admission control in both directions: an eventual write to a strong key and a strong write to an eventual key return `FAILED_PRECONDITION` without mutation. Causal replica tests now replay a three-record dependency chain in duplicate, reverse delivery order and verify that the dependency queue drains only after its prerequisites arrive. These checks support the fixed write-class invariant in §4.4, while durable request de-duplication and live partition histories remain open.

`cmd/meridian-openload` now consumes the pre-registered `bench/workloads/mixed.json` manifest and schedules requests at a target offered rate with a bounded in-flight queue. It writes scheduled, invoked, and completed times, every outcome, returned weak-path versions, causal contexts, Raft indices, and CAS results to JSONL, and uses the public leader-aware client for strong operations. `cmd/meridian-staleness` uses those fields to identify reads that miss a completed, non-concurrent write in the same history and reports the elapsed lower bound; it excludes concurrent weak writes instead of manufacturing a total order. It cannot yet measure acknowledgement-to-visibility delay or replace a full PBS-style staleness analysis. The driver is a harness component, not a Q1 result: it has not yet been run against a retained deployment, external baseline, or fault schedule.
